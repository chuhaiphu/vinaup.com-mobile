...
export interface OrgCustomerSelectModalProps {
  ...
  isBusy: boolean;
  onConfirm: (payload: { organizationCustomerId: string }, callback: () => void) => void;
}
...
export function OrgCustomerSelectModal({
  ...
  const handleConfirm = () => {
    if (pendingSelection?.type === 'internal') {
      onConfirm({ organizationCustomerId: pendingSelection.customerId }, onCloseRequest);
      return;
    }
    ...
    if (existingCustomer) {
      onConfirm({ organizationCustomerId: existingCustomer.id }, onCloseRequest);
      return;
    }
    ...
    createOrgCustomer(selectedOrg, {
      onSuccess: (created) => {
        ...
        if (created) {
          onConfirm({ organizationCustomerId: created.id }, onCloseRequest);
        }
      },
      onError: () => {
