...
export function TourSettlementCancelLogModalContent({
  ...
  const {
      ...
      getTourSettlementLogsByTourSettlementIdApi(tourData.tourSettlement?.id || ''),
    {
      ...
      tags: [`tour-settlement-cancel-logs-${tourData.tourSettlement?.id}`],
    }
  );

  useEffect(() => {
    if (!tourData.tourSettlement?.id) {
      return;
    }
    fetchCancelLogs();
  }, [tourData.tourSettlement?.id, fetchCancelLogs]);
  ...
  const handlePressLog = (log: TourSettlementCancelLogResponse) => {
    router.push({
      pathname:
        '/(protected)/tour-settlement-cancel-log-detail/[tourSettlementCancelLogId]',
      params: {
