...
interface GroupedReceiptPayment {
  label: string;
  items: ReceiptPaymentResponse[];
}

interface TicketSummarySnapshot {
  totalReceipt: number;
  totalPayment: number;
  totalTaxPay: number;
...
function buildReceiptPaymentRows(items: ReceiptPaymentResponse[]): string {
  return items
    .map((item) => {
...
function buildHtml(input: TourCalculationCancelLogPdfInput): string {
  const {
    cancelLog,
    organization,
    snapshotTour,
      ...
      <body>
        <div class="content-wrap">
          <div class="header-title-row">
            <div class="main-title">Tính giá</div>
            ${avatarBlock}
          </div>

          <div class="sub-header-row">
            <div class="org-name">${escapeHtml(organization?.name || '-')}</div>
            <div class="date-text">${escapeHtml(formatDateTime(cancelLog.createdAt))}</div>
          </div>
          ...
          <div class="section-block page-break-avoid">
            <div class="table-header-row">
              <div class="summary-col-1 summary-header">Tổng (Dự kiến) = ${totalExpectedCount}</div>
          ...
          <div class="signature-section page-break-avoid">
            ...
            <div class="sig-row-space">
              <div class="cancel-text">Hủy bởi: ${escapeHtml(cancelLog.canceledByUser?.name || '-')}</div>
              <div class="sig-date-text-italic">${escapeHtml(formatDateTime(cancelLog.createdAt))}</div>
            </div>
